...
Write a function subset a b that returns true iff the set represented by the list a is a subset of the set represented by the list b. This function should work with lists of any type: that is, the type of subset should be a generalization of 'a list -> 'a list -> bool.

Write a function equal_sets that returns true iff the represented sets are equal.
...
OK, now for the real work. Write a function filter_reachable g that returns a copy of the grammar g with all unreachable rules removed. This function should preserve the order of rules: that is, all rules that are returned should be in the same order as the rules in g.
...
The command ocaml should output the version number 3.12.1.
...
#
© 2006–2011 Paul Eggert. See copying rules.
$Id: hw1.html,v 1.59 2011/09/22 06:47:31 eggert Exp $